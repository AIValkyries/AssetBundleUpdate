...
class SheetInterpreter:
    ...
    def __init__(self, xls_file_path, sheet_name,csBelongFlags):
        self._xls_file_path = xls_file_path
        self._sheet_name = sheet_name
        self._cs_belong=csBelongFlags
        ...
        self._pb_file_name = "data_config_" + sheet_name.lower() + ".proto"
    ...
    def Interpreter(self) :
        ...
        #proto的包名
        self._output.append("package data_config_;\n")
    ...
    def _FieldDefine(self, repeated_num) :
        ...
        field_cs_rule = str(self._sheet.cell_value(FILED_CS_BELONG_ROW,self._col)).lower()
        field_rule = str(self._sheet.cell_value(FIELD_RULE_ROW, self._col))

        if field_cs_rule != self._cs_belong and field_cs_rule != "cs" and field_cs_rule != "":
            self._col += 1
            return
...
class DataParser:
    ...
    def __init__(self, xls_file_path, sheet_name,csBelongFlags):
        ...
        try:
            self._module_name = "data_config_" + self._sheet_name.lower() + "_pb2"
    ...
    def _WriteData2File(self, data) :
        file_name = "data_config_" + self._sheet_name.lower() + ".data"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()

    def _WriteReadableData2File(self, data) :
        file_name = "data_config_" + self._sheet_name.lower() + ".txt"
        file = open(file_name, 'wb+')
        file.write(data)
        file.close()
...
if __name__ == '__main__' :
    ...
    # option 0 生成proto和data 1 只生成proto 2 只生成data 3 cs_belong
    op = 0
    if len(sys.argv) > 4 :
        op = int(sys.argv[3])
    ...
    cs_belong = sys.argv[3]

    if op == 0 or op == 1:  #生成proto
        try :
            tool = SheetInterpreter(xls_file_path, sheet_name,cs_belong)
            tool.Interpreter()
        except BaseException, e :
    ...
    if op == 0 or op == 2:   #生成data
        try :
            parser = DataParser(xls_file_path, sheet_name,cs_belong)
            parser.Parse()
        except BaseException, e :
            print "Parse Failed!!!"
            print e
